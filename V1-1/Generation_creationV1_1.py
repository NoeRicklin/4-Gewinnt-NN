from NN_SetupV1_1 import bot_count, version
    ...
    for layer in new_parameters:
        for node in layer:
            for coefficient in node[0]:
                adjusted_parameters += str(float(coefficient) + random.random() - .5) + ","
            adjusted_parameters = adjusted_parameters.rstrip(",") + " "
            adjusted_parameters += str(float(node[1]) + random.random() - .5) + "|"
        adjusted_parameters = adjusted_parameters.rstrip("|") + "\n"
    adjusted_parameters = adjusted_parameters.rstrip("\n")

    NN_file = open(os.path.dirname(__file__) + f'\\bot_parametersV1-1\\Bot{bot_index}.txt', "w")
