...
import csv
from Generation_creationV6 import next_generation, num_fittest, bot_count, version
...
def play_game(bot1, bot2):
    ...
    while True:
        ...
        prev_stacked_amount = win_types["Stapel"]
        # Check for win
        if test_win(gameState, new_stone_pos, cur_player, win_types):
            cur_stacked_amount = win_types["Stapel"]
            if cur_stacked_amount != prev_stacked_amount:
                global stacked_win
                stacked_win = True
    ...
    for bot1 in range(bot_count):
        for bot2 in range(bot_count):
            ...
            if winner == 1:
                if stacked_win:
                    bot_fitness[bot1] -= 20
                    bot_fitness[bot2] -= 25 - 0.5 * moves
                else:
                    bot_fitness[bot1] += 10 + moves
                    bot_fitness[bot2] -= 20 - 0.5 * moves

            elif winner == -1:
                if stacked_win:
                    bot_fitness[bot2] -= 20
                    bot_fitness[bot1] -= 25 - 0.5 * moves
                else:
                    bot_fitness[bot2] += 12 + moves
                    bot_fitness[bot1] -= 20 - 0.5 * moves
